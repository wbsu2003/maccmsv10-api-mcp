...
async function handleApiRequest(url) {
    ...
    try {
        ...
        // 处理新的 /api/episodes 路径
        if (url.pathname.startsWith('/api/episodes/')) {
            // 这个请求应该直接转发到后端API
            // 不需要在前端处理，直接让请求通过
            console.log('API请求转发到后端:', url.pathname);
            return null; // 返回null表示不在前端处理此路径
        }

        console.warn('未知的API路径:', url.pathname);
...
(function() {
    ...
    window.fetch = async function(input, init) {
        ...
        // 对于 /api/episodes 路径，直接调用原生fetch，不进行拦截处理
        ...
        if (requestUrl.pathname.startsWith('/api/')) {
            ...
            try {
                ...
                if (data === null) {
                    // 如果handleApiRequest返回null，表示不在前端处理，调用原生fetch
                    return originalFetch.call(this, input, init);
                }
