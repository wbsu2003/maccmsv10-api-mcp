...
async function initializePageContent() {
    ...
    const isNewArchitecture = videoId && movieTitle && !videoUrl;
    ...
    // 从localStorage获取数据
    currentVideoTitle = title || movieTitle || localStorage.getItem('currentVideoTitle') || '未知视频';
    ...
    // 初始化播放器
    if (videoUrl) {
        // 传统模式：直接使用URL参数中的视频地址
        await initPlayer(videoUrl);
    } else if (isNewArchitecture) {
        // 新URL优化架构：等待video-player.js加载分集数据
        console.log('使用新URL优化架构，等待分集数据加载...');
        
        // 监听分集数据加载完成事件
        const handleEpisodesLoaded = async (event) => {
            console.log('接收到分集数据加载完成事件:', event.detail);
            const { episodes, currentIndex } = event.detail;
            ...
            if (episodes && episodes.length > 0) {
                ...
                if (currentEpisode && currentEpisode.url) {
                    console.log('初始化播放器，当前分集:', currentEpisode);
                    currentVideoUrl = currentEpisode.url;
                    await initPlayer(currentEpisode.url);
            ...
            // 移除事件监听器
            document.removeEventListener('episodesLoaded', handleEpisodesLoaded);
        };
        
        // 添加事件监听器
        document.addEventListener('episodesLoaded', handleEpisodesLoaded);
        
        // 设置超时，防止无限等待
        setTimeout(() => {
            document.removeEventListener('episodesLoaded', handleEpisodesLoaded);
            if (!window.episodes || window.episodes.length === 0) {
                console.warn('分集数据加载超时，可能是单集视频');
                showError('分集数据加载超时，请检查网络连接或稍后重试');
            }
        }, 10000);
    } else {
        showError('无效的视频链接');
...
async function initPlayer(videoUrl) {
    ...
    if (videoUrl.includes('.m3u8')) {
        try {
            // 直接使用PROXY_URL，不添加复杂的鉴权
            proxiedUrl = PROXY_URL + encodeURIComponent(videoUrl);
            console.log('使用代理URL播放:', proxiedUrl);
        } catch (error) {
            console.warn('代理URL生成失败，使用原始URL:', error);
            proxiedUrl = videoUrl;
        }
    }
